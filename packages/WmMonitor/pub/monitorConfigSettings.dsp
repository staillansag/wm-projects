...
<SCRIPT>
	...
	alertFieldDefaultSet();
	}else if(isEmptyField('myWebmethodsPath')){
	alertFieldDefaultSet();
 ...
 if ( !validateMWSRootContextPath('myWebmethodsPath') ) {
          alert('The MWS Root Context must contain only alphanumeric, period, hyphen and underscore characters.');
          retVal = false;
        }
 ...
 // Vaidate the MWS Root Context String
  function validateMWSRootContextPath(fieldName) {
   		theForm = document.config;
        valueField = eval('theForm.' + fieldName);
        theValue = valueField.value.trim();
     if (theValue != null && theValue.length > 0) {
        if (theValue.match(/[\w\.\-]+/g) != theValue) {
           return false;
        } else return true;
     } else return true;
  }
  ...
  <TABLE width="100%">
	...
	<SCRIPT>swapRows();</SCRIPT>
          ...
          <SCRIPT>swapRows();</SCRIPT>
    		<TR>
              <SCRIPT>writeTDnowrap("row");</SCRIPT>
              	MWS Root Context Path</TD>
            <SCRIPT>writeTDnowrap("row-l");</SCRIPT>
                <INPUT NAME="myWebmethodsPath"  VALUE="%value myWebmethodsPath%">
            </TD>    
          </TR>
	<SCRIPT>swapRows();</SCRIPT>
    		<TR>
